...
#To do -
1. Finish commenting
2. plot AUC curve and conf matrix together
3. add conclusion
...
To correct this imbalance I will generate new examples of the minority class using nearest neighbors by using **themis::step_smote**
...
default_rec1 <- 
  ...
  step_dummy(all_predictors(),one_hot = TRUE) %>% step_smote(Default)
...
Our initial model has an **accuracy** score of `r metric_df[metric_df$.metric == "accuracy",]` and and **roc_auc** score of `r metric_df[metric_df$.metric == "roc_auc",]`
...
metric_df <-
  bind_rows(bind_rows(
    ...
    roc_auc(glm2_pred,Default,.pred_0)) %>%
...
metric_df %>%
  ...
  scale_y_continuous(limits = c(0.8,0.95),oob = rescale_none) +
  labs(y = "Score", x = "Model",
       title = "**Lasso regression** slightly outperforms other models") +
  guides(fill = guide_legend(title = "**Metric**"))

```


As our logistic regression model containing all the features is has the best fit for the data, we will use its estimate for default when calculating risk classes using the prediction data from the chosen model
