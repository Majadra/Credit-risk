...
- Preprocessing steps to be added:
  ...
  3. address the fact that indivuals that have defaulted are underrepresented:
...
# m = glm(Default ~ ., family = binomial(), data = train)
# card = scorecard(bins, m)
# prob = predict(m, train, type='response')

formula1 = paste(names(train)[c(1,3:12)],collapse = " + ")

default_rec1 <- recipe(data = train,formula = paste0("Default ~ ",formula1,sep = "")) %>%
...
Our initial model has an **accuracy** score of 0.797 and and **roc_auc** score of 0.843
...
metric_df <-
  bind_rows(bind_rows(
    accuracy(glm_pred,Default,.pred_class),
    roc_auc(glm_pred,Default,.pred_0)) %>%
...
train_fold <- train %>% vfold_cv(5,strata = Default)
...
The lasso model performs worse then the simple logistic model
...
rf_grid <-
  crossing(mtry = c(9:15),min_n = c(seq(1,25,5)),trees = c(500,1000))
